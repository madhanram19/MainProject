...
const kycSchema = Yup.object().shape({
  ...
  aadharNumber: Yup.string()
    ...
    .matches(/^[0-9]{16}$/g, 'Enter valid Aadhar Number')
    .trim(),
...
const BankDetailspage = () => {
  ...
  const [kycErroStatus, setKycErrorStatus] = useState([]);

  const isAdminRejected = kycErroStatus.some((data) => data.message);

  console.log({ isAdminRejected, kycErroStatus });

  const getRejectedStatus = (field, message) => {
    if (!isAdminRejected) return null;
    const rejectionMessage = kycErroStatus.filter(
      (data) => data.field === field
    )?.[0]?.message;
    if (!message) return !rejectionMessage;
    if (rejectionMessage) return <p>{rejectionMessage}</p>;
    return null;
  };
  ...
  useEffect(() => {
    const fetchKycData = async () => {
      try {
        ...
        setKycErrorStatus(kycDetails.errorStatus);
            ...
            <div>
              <ul className="steps">
                <li className="steplevel">
                  <span>ID Proof 1</span>
                </li>
                <li className="steplevel processing">
                  <span>ID Proof 2</span>
                </li>
                <li className="steplevel ">
            ...
            <form onSubmit={handleSubmit(handleKycVerifycation)}>
              <div class="IDproof_sec">
                ...
                <>
                  ...
                  ) : waitingStatus && !isAdminRejected ? (
                    <span className="small d-inline-block mt-2 ms-2 text-primary">
                      <MdPendingActions /> Pending (waiting for approval)
                    </span>
                  ) : (
                    <>
                      <div class="row">
                        <div class="col-lg-4">
                          <div class="form-inline mb-5">
                            <label class="mr-3 text-gray fs-16 fw-400">
                              Select Country
                            </label>
                            <select
                              className={`form-control py-0 ${
                                errors.selectCountry ? 'is-invalid' : ''
                              }`}
                              type="text"
                              name="selectCountry"
                              disabled={getRejectedStatus('selectCountry')}
                              {...register('selectCountry')}
                              class="custom-select"
                              style={{ minWidth: '230px', height: '50px' }}
                            >
                              <option>India</option>
                            ...
                            {getRejectedStatus('selectCountry', 'message')}
                            ...
                            <div class="form-group formInputs mb-4">
                              <div class="input-group iconinput">
                                ...
                                <input
                                  name="aadharName"
                                  type="text"
                                  disabled={getRejectedStatus('aadharName')}
                                  {...register('aadharName')}
                                  className={`form-control ${
                                    errors.aadharName ? 'is-invalid' : ''
                                  }`}
                                  placeholder="Enter your Name as per your aadhar card"
                                />
                                {getRejectedStatus('aadharName', 'message')}
                                <div
                            ...
                            <div class="form-group formInputs mb-4">
                              <div class="input-group iconinput">
                                ...
                                <input
                                  name="aadharNumber"
                                  type="text"
                                  disabled={getRejectedStatus('aadharNumber')}
                                ...
                                />
                                {getRejectedStatus('aadharNumber', 'message')}
                                <div
                                  className="invalid-feedback"
                                  style={{
                                    marginLeft: '10%',
                        ...
                        <div class="col-lg-8">
                          <div class="fs-16 fw-600 mb-2">
                            Upload Aadhar Card
                          </div>
                          <div class="fs-14 fw-400 text-gray mb-4">
                            * Format: JPEG (.jpg or .jpeg) or PNG format, Size:
                            less than 5MB
                          </div>
                            ...
                            <div class="card-body px-5 pt-4 pb-0">
                              <div className="row">
                                <div className="col-md-6">
                                  {getRejectedStatus('aadharFront', 'message')}
                                  <DropFileInput
                                    onFileChange={(files) =>
                                ...
                                <div className="col-md-6">
                                  {getRejectedStatus('aadharBack', 'message')}
                                  <DropFileInput
                                    onFileChange={(files) =>
                                      backSideImage(files)
                                    }
                      ...
                      <div class="text-center fs-14 fw-400 text-gray mb-4">
                        ...
                        To change the country later, you can raise a ticket to
                        and ask us to reset your KYC.
                      </div>
                    ...
                    <div class="card-body">
                      <div class="d-flex">
                        ...
                        <div>
                          ...
                          <button
                            class="btn bg-transparent fs-14 fw-400"
                            type="button"
                          >
                            <img src={edit_blue} /> Edit
                          </button>
                          <button
                            class="btn bg-transparent fs-14 fw-400"
                            type="button"
                          >
                            <img src={delete_blue} /> Delete
                          </button>
                        </div>
                      </div>
                    </div>
